...
'''
# TODO: Finish up the implementation.
...
def reduce_word(word, word_list):
    if word in memo:
        return memo[word]
    resultant_words = []
    for child in get_children(word, word_list):
        if reduce_word(child, word_list):
            resultant_words.append(child)

    memo[word] = resultant_words
